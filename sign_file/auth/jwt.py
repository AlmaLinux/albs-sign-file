...
import jwt
from sign_file.api.schema import JWT_Token
...
class JWT:
    ...
    def encode(self, user_id: int, email: str) -> JWT_Token:
        ...
        return JWT_Token(user_id=user_id,
                         email=email,
                         exp=int(exp.timestamp()),
                         token=encoded_token)

    def decode(self, token: str) -> JWT_Token:
        decoded_token = jwt.decode(token, self.secret, algorithms=[
            self.hash_algoritm, ])
        return JWT_Token(user_id=decoded_token['user_id'],
